...
def generate_launch_description():
    moveit_config = (
        MoveItConfigsBuilder("piper_with_gripper")
        .robot_description(file_path="config/piper.urdf.xacro")
        .to_moveit_configs()
    )
    # Get parameters for the Servo node
    servo_params = (
        ParameterBuilder("piper_with_gripper_moveit_config")
        .yaml(
            parameter_namespace="moveit_servo",
            file_path="config/piper_simulated_config.yaml",
        )
        .to_dict()
    )
    ...
    rviz_config_file = (
        ...
        + "/config/piper_demo_rviz_config.rviz"
    )
    rviz_node = Node(
        package="rviz2",
        executable="rviz2",
    ...
    ros2_control_node = Node(
        ...
        parameters=[
            moveit_config.robot_description,
            os.path.join(
                get_package_share_directory("piper_with_gripper_moveit_config"),
                "config",
